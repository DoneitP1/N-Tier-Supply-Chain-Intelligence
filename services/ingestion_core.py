...
async def process_contract(
    ...
    if user:
        audit = AuditLog(user_id=user.id, action="ingest_contract", target_node=contract.supplier.name)
        db_sql.add(audit)
        await db_sql.commit()
    
    return {"message": f"Contract for {contract.supplier.name} mapped to Knowledge Graph"}
